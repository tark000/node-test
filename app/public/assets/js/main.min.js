...
myApp.value("messageFormatter",function(e,t,n){return e.toLocaleTimeString()+" - "+t+" - "+n+"\n"});

var myApp=angular.module("myApp",["ngRoute","ngCookies","ngResource","ngSanitize","btford.socket-io"]).value("nickName","anonymous");myApp.config(["$routeProvider","$locationProvider",function(e,a){e.when("/",{templateUrl:"/assets/tmpl/home.html",controller:"HomeController"}).when("/create-note",{templateUrl:"/assets/tmpl/createNote.html",controller:"CreateNoteController"}).otherwise({redirectTo:"/"}),a.html5Mode({enabled:!0,requireBase:!1})}]),myApp.controller("SocketCtrl",["$log","$scope","chatSocket","messageFormatter","nickName",function(e,a,o,t,s){a.nickName=s,a.messageLog="Ready to chat!",a.sendMessage=function(){var n=a.message.match("^/nick (.*)");if(angular.isDefined(n)&&angular.isArray(n)&&2===n.length){var r=s;s=n[1],a.message="",a.messageLog=t(new Date,s,"nickname changed - from "+r+" to "+s+"!")+a.messageLog,a.nickName=s}e.debug("sending message",a.message),o.emit("message",s,a.message),e.debug("message sent",a.message),a.message=""},a.$on("socket:broadcast",function(o,s){return e.debug("got a message",o.name),s.payload?void a.$apply(function(){a.messageLog=t(new Date,s.source,s.payload)+a.messageLog}):void e.error("invalid message","event",o,"data",JSON.stringify(s))})}]),myApp.factory("chatSocket",["socketFactory",function(e){var a=e();return a.forward("broadcast"),a}]);
myApp.controller("CreateNoteController",["$scope",function(o){console.log("create note")}]);
myApp.controller("HomeController",["$scope",function(o){o.test="Test text from angular",console.log("123asd")}]);
"use strict";myApp.factory("chatSocket",["socketFactory",function(t){var r=t();return r.forward("broadcast"),r}]);